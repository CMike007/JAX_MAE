...
def generate_order(TypeDict, startTime, tickSize,
    ...
    order_size = np.random.randint(20, 200)
...
HoursOfData = 1
params = {
    "LOBsize":int(396000 * HoursOfData),
    "InitBid":210000,
    "InitAsk":210830,
    "TickSize":10,
    "StandingVol":4000,
    "TypeDict":{1: 0.5, 2: 0.15, 3: 0.15, 4: 0.2},
    "StartTime": 0,
    "Level": 10,
    "mean": 6,
    "std": 2,
    ...
    # handling SELL Limit Orders
        ...
        if order['Size'] < LOB[i-1][1]:
            ...
            for j in range(0, 4*params["Level"]-5, 4):
                newLine[j] = newLine[j + 4]
                newLine[j + 1] = newLine[j + 5]
            newLine[4*params["Level"]-4] = newLine[12] + tick
            newLine[4*params["Level"]-3] = order['Size']
        LOB[i] = newLine
        ...
        if order['Size'] < LOB[i-1][3]:
            ...
            for j in range(2, 4*params["Level"]-3, 4):
                newLine[j] = newLine[j + 4]
                newLine[j + 1] = newLine[j + 5]
            newLine[4*params["Level"]-2] = newLine[14] - tick
            newLine[4*params["Level"]-1] = order['Size']
        LOB[i] = newLine
...
#since messageData should have data for 5 days:
    # --> SYN lenght / 5 / 24 * 5.5
#Calculations to the arrival time etc. of orders  
...
print(f'LOBSTER: Time of first Message: {RealDataMSG[0, 0]}')
print(f'LOBSTER: Time of last Message: {RealDataMSG[-1, 0]}')
...
plot_mid_price_with_volume_levels(LOBData[:12000])
...
plot_mid_price_with_volume_levels(RealDataLOB[1000:3000])
