...
sys.path.append('/cs/student/projects3/cf/2023/mbirkenz/JAX_MAE/purejaxrl-main')
sys.path.append('/cs/student/projects3/cf/2023/mbirkenz/JAX_MAE/AlphaTrade-jaxV3')

from jax import config

from gymnax_exchange.jaxen.exec_env_Sigmoid import ExecutionEnv
...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            if config.get("DEBUG"):
                ...
                def callback(metric):
                    ...
                    deltaP = info["price_delta_rm"][info["returned_episode"]]
                    ...
                    BuyExecuted_Step = info["BuyExecuted_Step"][info["returned_episode"]]
                    SellExecuted_Step = info["SellExecuted_Step"][info["returned_episode"]]
                            ...
                            wandb.log(
                                {
                                    ...
                                    "BuyExecuted_Step":BuyExecuted_Step[t],
                                    "SellExecuted_Step":SellExecuted_Step[t],
                                }
                            )
        ...
        # TAgent - INIT NETWORK
        TAgent_network = ActorCriticS5(1, config=config)
        rng, _rng = jax.random.split(rng)
        init_TAgent_x = (
            jnp.zeros(
                (1, config["NUM_ENVS"], *env.observation_space(env_params).shape)
...
if __name__ == "__main__":
    ...
    ppo_config = {
        "NUM_ENVS": 300,            # reduced from 500
        "TOTAL_TIMESTEPS": 25e6,    # reduced from 1e8 (last run #dandy-leaf-28 was done with 1e6 --> 2h runtime)
        "NUM_MINIBATCHES": 5,       # (they also tried 4 instead)
        ...
        "NUM_STEPS": 360,           # brought down from 455
        "ENV_NAME": "alphatradeExec-v0",
        "WINDOW_INDEX": -1,
        "DEBUG": True,
        "wandbOn": True,
        "ATFOLDER": '/cs/student/projects3/cf/2023/mbirkenz/JAX_MAE/AlphaTrade-jaxV3/',
        "PARAMSFOLDER": '/cs/student/projects3/cf/2023/mbirkenz/JAX_MAE/AlphaTrade-jaxV3/params/',
        "RESULTS_FILE":"/cs/student/projects3/cf/2023/mbirkenz/JAX_MAE/AlphaTrade-jaxV3/results_file_"+f"{timestamp}",
        "CHECKPOINT_DIR":"/cs/student/projects3/cf/2023/mbirkenz/JAX_MAE/AlphaTrade-jaxV3/checkpoints_"+f"{timestamp}",
        "PICTURE_DIR":"/cs/student/projects3/cf/2023/mbirkenz/JAX_MAE/AlphaTrade-jaxV3/plot"+f"{timestamp}",
        "Plot": False,
        "OOE": {
            "TASKSIDE":'sell',
            "TASK_SIZE":40000,
            ...
            "ACTION_TYPE":"pure",
            # "ACTION_TYPE":"delta",
            "LR": 7e-5,         #2.5e-5, --> Just wanted to play around with it a bit...
            "CLIP_EPS": 0.2,
            "GAMMA": 0.99,
            "GAE_LAMBDA": 0.95,
            "REWARD_LAMBDA":1,
            ...
            "MAX_GRAD_NORM": 2.0,
            "ANNEAL_LR": True,
            "Episode_Time": 60*30, #60seconds times 30 minutes = 1800seconds
        },
        "TAgent": {
            "TASK_SIZE":200,
            "LR": 7e-5,         #2.5e-5, --> Just wanted to play around with it a bit...
    ...
    with open(ppo_config["PARAMSFOLDER"] + params_file_name + '_OOE', 'wb') as f:
        f.write(flax.serialization.to_bytes(params_OOE))
        print(f"pramas of the Optimal Order Execution Agent saved")
    
    # Save the params to a file using flax.serialization.to_bytes
    with open(ppo_config["PARAMSFOLDER"] + params_file_name + '_TA', 'wb') as f:
