...
class ExecutionEnv(BaseLOBEnv):
    def __init__(self, alphatradePath, window_index, OOE_data, TAgent_data = None, RLAgent3 = None):
        ...
        '''
        Max Time is hardcoded right now to the max time in a cycle, currently the max time should then be set to the
        desired maximum time of an episode in seconds times ten (MaxTime = time_in_sec * 10)
        '''
    ...
    def step_env(
        ...
        buy_obj = jnp.minimum(((self.task_size_TA * state.step_counter / state.max_steps_in_episode) - ((abs(state.quant_BUY_TA - (state.step_counter / state.max_steps_in_episode) * self.task_size_TA))**1.3)).astype(jnp.int32), 30)
        sell_obj = jnp.minimum(((self.task_size_TA * state.step_counter / state.max_steps_in_episode) - ((abs(state.quant_SELL_TA - (state.step_counter / state.max_steps_in_episode) * self.task_size_TA))**1.3)).astype(jnp.int32), 30)

        Flow_TA = (buy_obj + sell_obj) #* mid_price * 0.000001
        Flow_TA = Flow_TA * 0.00001 * mid_price

        #Just for tracking purposes:
        PnL = PnL_TA(self.gamma)
    ...
    def reset_env(
        ...
        def stateArray2state(stateArray):
            ...
            omega = jax.random.uniform(key, (1,), minval=0.60, maxval=0.80)[0];
