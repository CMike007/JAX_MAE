...
class EnvParams:
    ...
    episode_time: int =  60*30 #60seconds times 30 minutes = 1800seconds * 10
...
class ExecutionEnv(BaseLOBEnv):
    def __init__(self, alphatradePath, window_index, OOE_data, TAgent_data = None, RLAgent3 = None):
        ...
        # Including Benchmark models
        self.TWAP_vol = OOE_data['TASK_SIZE'] / OOE_data['NUM_STEPS']
    ...
    def step_env(
        ...
        action_msg_TWAP = jnp.array(
            [[1, self.Task, self.TWAP_vol, state.best_bids[-1, 0] - self.tick_size, self.trader_unique_id+1,  self.trader_unique_id+1, self.episodeTime, 1],
            [1, self.Task, self.TWAP_vol / 2, state.best_bids[-1, 0]- 2*self.tick_size, self.trader_unique_id+2,  self.trader_unique_id+2, self.episodeTime, 1],
            [1, self.Task, self.TWAP_vol / 4, state.best_bids[-1, 0]- 3*self.tick_size, self.trader_unique_id+3,  self.trader_unique_id+3, self.episodeTime, 1]]).astype(jnp.int32)

        #jax.debug.print('action_msg_TWAP entry 1: {}', action_msg_TWAP[0][0])
        #jax.debug.print('action_msg_TWAP entry 2: {}', action_msg_TWAP[0][1])
        #jax.debug.print('action_msg_TWAP entry 3: {}', action_msg_TWAP[0][2])
        #jax.debug.print('action_msg_TWAP entry 4: {}', action_msg_TWAP[0][3])
        # jax.debug.print('action_msg_TWAP entry 5: {}', action_msg_TWAP[0][4])
        # jax.debug.print('action_msg_TWAP entry 6: {}', action_msg_TWAP[0][5])
        # jax.debug.print('action_msg_TWAP entry 7: {}', action_msg_TWAP[0][6])
        # jax.debug.print('action_msg_TWAP entry 8: {}', action_msg_TWAP[0][7])
        ...
        # jax.debug.print('data_messages entry 1: {}', data_messages[0][0])
        # jax.debug.print('data_messages entry 2: {}', data_messages[0][1])
        # jax.debug.print('data_messages entry 3: {}', data_messages[0][2])
        # jax.debug.print('data_messages entry 4: {}', data_messages[0][3])
        # jax.debug.print('data_messages entry 5: {}', data_messages[0][4])
        # jax.debug.print('data_messages entry 6: {}', data_messages[0][5])
        # jax.debug.print('data_messages entry 7: {}', data_messages[0][6])
        # jax.debug.print('data_messages entry 8: {}', data_messages[0][7])
        ...
        messages_TA = jax.vmap(getActionMsgsTA, in_axes=(0, None, None, 0, 0, 0, 0))(actions_TA_, state, params, state.quant_BUY_TA_arr, state.quant_SELL_TA_arr, state.task_to_execute_TA_arr, IDs)

        total_messages = jnp.concatenate([jnp.squeeze(messages_TA), data_messages, action_msg_TWAP])

        #jax.debug.print('messages_TA: {}', messages_TA[0])
        ...
        #jax.debug.print('OOETrades: {}', OOETrades)
        ...
        def PnL_TA(gamma, inventory, TAgentTrades, direction):
            ...
            delta_PnL = (avgBuyPrice- mid_price) * TAgentTrades[:, 1].sum() + delta_PnL_Inventory * direction
            return (delta_PnL + delta_PnL_Inventory - gamma * abs(delta_PnL_Inventory)).astype(jnp.int32), avgBuyPrice
        ...
        def settleTrades(trader_id, executed_trades, task_size, quantBUY, quantSELL, inventory, gammma=0.95, omega=0.75):
            ...
            PnL, avgPrice = PnL_TA(gammma, inventory, TAgentTrades, direction)
        ...
        state = EnvState(asks,bids,trades,bestasks,bestbids,state.init_time,time,state.customIDcounter+self.n_actions,state.window_index,\
            ...
            state.max_steps_in_episode, RelPriceDelta_ST, RelPriceDelta_LT, self.task_size_TA, quant_buy_arr, quant_sold_arr, quant_inventory_arr, mid_price,\
        ...
        return self.get_obs(state,params), state, [rewardOOE, _], done,\
            ...
            "RelPriceDelta": state.price_delta_LT + state.price_delta_ST,\
