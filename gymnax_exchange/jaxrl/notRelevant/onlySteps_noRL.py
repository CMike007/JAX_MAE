...
def make_train(config):
    ...
    '''
    original (adjusted by Mike 12.06)
    env= ExecutionEnv(config["ATFOLDER"],config["TASKSIDE"],config["DEBUG"])
    --> included a 0 for window Index (as done in the tryout example at the end of exec_env.py)
    '''
    env= ExecutionEnv(config["ATFOLDER"],config["TASKSIDE"],0,config["DEBUG"])
...
if __name__ == "__main__":
    ...
    ppo_config = {
        "LR": 2.5e-4,
        "NUM_ENVS": 10, # orignially 1000 (Mike 12.06)
        "NUM_STEPS": 10,
        "TOTAL_TIMESTEPS": 5e4, # originally 5e4 (Mike 12.06)
        "UPDATE_EPOCHS": 4,
        "NUM_MINIBATCHES": 4,
        "GAMMA": 0.99,
        "GAE_LAMBDA": 0.95,
