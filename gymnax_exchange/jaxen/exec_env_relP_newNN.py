...
class ExecutionEnv(BaseLOBEnv):
    ...
    def step_env(
        ...
        #jax.debug.print("trades executed: {}", trades)
        #jax.debug.print("asks: {}", asks)
        ...
        buy_obj = 3 * (self.task_size_TA * state.step_counter / state.max_steps_in_episode) - abs(state.quant_BUY_TA - ((state.step_counter / state.max_steps_in_episode) * self.task_size_TA).astype(jnp.int32))
        sell_obj = 3 * (self.task_size_TA * state.step_counter / state.max_steps_in_episode) - abs(state.quant_SELL_TA - ((state.step_counter / state.max_steps_in_episode) * self.task_size_TA).astype(jnp.int32))
        # OLD VERSION: sell_obj = 1/3 * (self.task_size_TA) - abs(state.quant_SELL_TA - ((state.step_counter / 200) * self.task_size_TA).astype(jnp.int32))
        # OLD VERSION: buy_obj = 1/3 * (self.task_size_TA) - abs(state.quant_BUY_TA - ((state.step_counter / 200) * self.task_size_TA).astype(jnp.int32))
        # OLD VERSION: ratio_obj = 1/3 * (self.task_size_TA) - abs(state.quant_SELL_TA - state.quant_BUY_TA)
        ratio_obj = (self.task_size_TA * state.step_counter / state.max_steps_in_episode) - abs(state.quant_SELL_TA - state.quant_BUY_TA)
        ...
        state = EnvState(asks,bids,trades,bestasks,bestbids,state.init_time,time,state.customIDcounter+self.n_actions,state.window_index,\
            ...
            state.max_steps_in_episode, price_delta + state.price_delta, jnp.maximum((self.task_size_TA * 2) - (state.quant_BUY_TA + state.quant_SELL_TA), 1), 
        ...
        return self.get_obs(state,params), state, [rewardOOE, rewardTAgent], done,\
            ...
            "PnL_cummulative": state.PnL_TA_cummulative+PnL,\
    ...
    def get_obs(self, state: EnvState, params:EnvParams) -> chex.Array:
        ...
        '''
        Approach:
        Exchange shallow inbalance with the incoming trades of the next episode
        '''
        data_messages=job.get_data_messages(params.message_data,state.window_index,state.step_counter)
        data_messages = data_messages.at[:, 3].add(state.price_delta)
        #jax.debug.print("Data Messages in get_obs: {}", data_messages[:5, 3])

        obsTA = jnp.concatenate((best_bids,best_asks,mid_prices,second_passives,spreads,timeOfDay,deltaT,jnp.array([sharesBUY]),jnp.array([sharesSELL]),\
            jnp.array([inventory_TA]), jnp.array([taskSize_TA]),jnp.array([executed_quant_TA]),data_messages[:, 3],jnp.array([state.step_counter])))
        ...
        def obsNorm_TAgent(obs):
            ...
            return jnp.concatenate((
                obs[:400]/(initPrice*2), # best_bids,best_asks,mid_prices,second_passives  TODO CHANGE THIS
                obs[400:500]/10000, # spreads
                obs[500:501]/100000, # timeOfDay
                obs[501:502]/1000000000, # timeOfDay
                obs[502:503]/10,# deltaT
                obs[503:504]/1000000000,# deltaT
                obs[504:505]/taskSize_TA, # executed Sell
                obs[505:506]/taskSize_TA, # executed Buy
                obs[506:507]/taskSize_TA, # inventory_TA
                obs[507:508]/(taskSize_TA*2), # taskSize_TA
                obs[508:509]/(taskSize_TA*2), # executed_quant
                obs[509:609]/(initPrice*2), # Incoming Msg Prices next episode
