...
class ExecutionEnv(BaseLOBEnv):
    ...
    def step_env(
        ...
        action_msg_TWAP = jnp.array(
            [[1, self.Task, self.TWAP_vol / 2, state.best_bids[-1, 0] - 3*self.tick_size, self.trader_unique_id+1,  self.trader_unique_id+1, self.episodeTime, 1],
            [1, self.Task, self.TWAP_vol / 2, state.best_bids[-1, 0]- 4*self.tick_size, self.trader_unique_id+2,  self.trader_unique_id+2, self.episodeTime, 1],
            [1, self.Task, self.TWAP_vol / 2, state.best_bids[-1, 0]- 4*self.tick_size, self.trader_unique_id+3,  self.trader_unique_id+3, self.episodeTime, 1]]).astype(jnp.int32)
