...
class ExecutionEnv(BaseLOBEnv):
    ...
    def step_env(
        ...
        buy_obj = jnp.minimum((self.task_size_TA * state.step_counter / 200) - (state.quant_BUY_TA - ((state.step_counter / state.max_steps_in_episode) * self.task_size_TA).astype(jnp.int32))**2, 0)
        sell_obj = jnp.minimum((self.task_size_TA * state.step_counter / 200) - (state.quant_SELL_TA - ((state.step_counter / state.max_steps_in_episode) * self.task_size_TA).astype(jnp.int32))**2, 0)
        ratio_obj = 0
        # soft-dream-99 - ratio_obj = jnp.minimum((self.task_size_TA * state.step_counter / 200) - (state.quant_SELL_TA - state.quant_BUY_TA)**2, 0)
        #expert-dragon-98 - buy_obj = 3 * (self.task_size_TA * state.step_counter / state.max_steps_in_episode) - abs(state.quant_BUY_TA - ((state.step_counter / state.max_steps_in_episode) * self.task_size_TA).astype(jnp.int32))
        #expert-dragon-98 - sell_obj = 3 * (self.task_size_TA * state.step_counter / state.max_steps_in_episode) - abs(state.quant_SELL_TA - ((state.step_counter / state.max_steps_in_episode) * self.task_size_TA).astype(jnp.int32))
        #expert-dragon-98 - ratio_obj = 3 * (self.task_size_TA * state.step_counter / state.max_steps_in_episode) - abs(state.quant_SELL_TA - state.quant_BUY_TA)
        ...
        Flow_TA = (buy_obj + sell_obj + ratio_obj) * mid_price * 0.0001
