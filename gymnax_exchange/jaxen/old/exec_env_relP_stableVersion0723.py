...
class ExecutionEnv(BaseLOBEnv):
    def __init__(self, alphatradePath, window_index, OOE_data, RLAgent2 = None, RLAgent3 = None):
    ...
    def step_env(
        ...
        state = EnvState(asks,bids,trades,bestasks,bestbids,state.init_time,time,state.customIDcounter+self.n_actions,state.window_index,\
            ...
            state.max_steps_in_episode,price_delta+state.price_delta,slippage_rm,price_adv_rm,price_drift_rm,vwap_rm)
    ...
    def reset_env(
        ...
        def stateArray2state(stateArray):
            ...
            return (state0,state1,state2,state3,state4,state5,state6,0,idx_data_window,state9,self.task_size,0,0,0,self.max_steps_in_episode_arr[idx_data_window],0,0,0,0,0)
