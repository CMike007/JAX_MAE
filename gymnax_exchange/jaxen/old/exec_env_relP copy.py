...
class ExecutionEnv(BaseLOBEnv):
    def __init__(self, alphatradePath, window_index, OOE_data, TAgent_data = None, RLAgent3 = None):
        ...
        self.trader_unique_id_TA = -198098
    ...
    def step_env(
        ...
        def reshape_action(actions, state, params):
            ...
            def twapV3(state, env_params, agentType):
                ...
                if agentType == 'OOE':
                    ...
                    remainedQuant = state.task_to_execute_TA - (state.quant_BUY_TA + state.quant_SELL_TA)
            ...
            def truncate_action(action, remainQuant, agentType):
                action = jnp.round(action).astype(jnp.int32).clip(0,self.task_size) if agentType == 'OOE' else jnp.round(action).astype(jnp.int32).clip(0,self.task_size_TA)
            ...
            action_OOE_ = get_base_action(state, 'OOE', params)  + action_space_clipping(actions[0],state.task_to_execute) if self.action_type=='delta' else action_space_clipping(actions[0], state.task_to_execute)
            action_TAgent_ = get_base_action(state, params, 'TA')  + action_space_clipping(actions[1],state.task_to_execute_TA)  if self.action_type=='delta' else action_space_clipping(actions[1], state.task_to_execute_TA)
            action_OOE = truncate_action(action_OOE_, state.task_to_execute-state.quant_executed, 'OOE')
            jax.debug.print('Inside Function Check: {}', action_TAgent_)
            action_TAgent = truncate_action(action_TAgent_, state.task_to_execute_TA - (state.quant_BUY_TA + state.quant_SELL_TA), 'TA')
            return [action_OOE, action_TAgent_]
            # the output here as far as i know is still a array with the same input dimension and only with the action that has the biggest magnitude, truncated to max the outstanding quantity open to trade
        '''
        ...
        def retain_max_only(arr):
            scaling_factor = (self.task_size_TA * 0.2)
            max_index = jnp.argmax(arr)
            result = jnp.zeros_like(arr)
            result = result.at[max_index].set(arr[max_index]*scaling_factor).astype(jnp.int32)
        ...
        def getActionMsgsTA(action: Dict, state: EnvState, params: EnvParams) -> chex.Array:
            ...
            #order_ids=jnp.ones((self_n_actions_TA,),jnp.int32)*(self.trader_unique_id_TA-state.customIDcounter) #Each message has a unique ID
            ...
            action_msgs=jnp.stack([types,sides,quants,prices,trader_ids,sides],axis=1)
            action_msgs=jnp.concatenate([action_msgs,times],axis=1)
            return action_msgs

        action_TAgent = retain_max_only(action_TAgent)
        ...
        def conditional_concatenate(action_TAgent, cnl_msgs, action_msgs_OOE, data_messages, state, params):
            ...
            total_messages = jax.lax.cond(jnp.sum(action_TAgent) == 0, true_fun, false_fun, operand=None)
            return total_messages

        total_messages = conditional_concatenate(action_TAgent, cnl_msgs, action_msgs_OOE, data_messages, state, params)

        #jax.debug.print('Lets check those conditional concatenated msgs: {}', total_messages)
        ...
        # TAgent
        mask_TAgent = (self.trader_unique_id_TA == executed[:, 2])
        ...
        #jax.debug.print('Executed Orders: {}', TAgentTrades)

        #jax.debug.print('Shares bough Trading Agent: {}', shares_bought)
        #jax.debug.print('Shares sold Trading Agent: {}', shares_sold)
        ...
        buy_obj = (self.task_size_TA) * abs(state.quant_BUY_TA - (state.step_counter / 200).astype(jnp.int32) * self.task_size_TA)
        sell_obj = (self.task_size_TA) * abs(state.quant_SELL_TA - (state.step_counter / 200).astype(jnp.int32) * self.task_size_TA)
        Flow_TA = (buy_obj + sell_obj) * mid_price * 0.001
        ...
        historical_prices = state.price_hist.at[state.step_counter].set(mid_price)
        buy_hist = state.buy_hist.at[state.step_counter].set(shares_bought)
        sell_hist = state.sell_hist.at[state.step_counter].set(shares_sold)
        OOE_hist = state.OOE_hist.at[state.step_counter].set(new_execution_OOE)
        ...
        state = EnvState(asks,bids,trades,bestasks,bestbids,state.init_time,time,state.customIDcounter+self.n_actions,state.window_index,\
            ...
            state.max_steps_in_episode, price_delta + state.price_delta, jnp.minimum((self.task_size_TA * 2) - (state.quant_BUY_TA + state.quant_SELL_TA), 0), 
            state.quant_BUY_TA + shares_bought, state.quant_SELL_TA+shares_sold, state.inventory_TA+shares_bought-shares_sold, mid_price,
            state.PnL_TA_cummulative + PnL, historical_prices, buy_hist, sell_hist, OOE_hist,
        ...
        return self.get_obs(state,params), state, [rewardOOE, rewardTAgent], done,\
            ...
            "PnL_cummulative": state.PnL_TA_cummulative,\
            "Price_history": historical_prices,\
            "Buy_history": buy_hist,\
            "Sell_history": sell_hist,\
            "OOE_history": OOE_hist,\
            }
    ...
    def reset_env(
        ...
        def stateArray2state(stateArray):
            ...
            return (
                ...
                self.task_size_TA, 0, 0, 0, 0, 0,
                jnp.zeros(205), jnp.zeros(205), jnp.zeros(205), jnp.zeros(205),
                0,0,0,0
            )
    ...
    def get_obs(self, state: EnvState, params:EnvParams) -> chex.Array:
        ...
        obsNorm_OOE=obsNorm_OOE(obs)
        obsNorm_TAgent=obsNorm_TAgent(obs)
        # jax.debug.breakpoint()
