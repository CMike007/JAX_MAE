...
class ExecutionEnv(BaseLOBEnv):
    ...
    def step_env(
        ...
        #jax.debug.print('action of TAgent {}', actions[1])
        ...
        action_msgs_TAgent = self.getActionMsgs(actions[1], state, params, 'TAgent')
        ...
        shares_sold = jax.lax.cond(actions[0][0] != 0, lambda x: TAgentTrades[:, 1].sum(), lambda x: 0, operand=None)
        shares_bought = jax.lax.cond(actions[0][1] != 0, lambda x: TAgentTrades[:, 1].sum(), lambda x: 0, operand=None)
        ...
        buy_obj = (self.task_size_TA * 0.2) - abs(state.quant_BUY_TA - (state.step_counter / state.max_steps_in_episode).astype(jnp.int32) * self.task_size_TA)
        sell_obj = (self.task_size_TA * 0.2) - abs(state.quant_SELL_TA - (state.step_counter / state.max_steps_in_episode).astype(jnp.int32) * self.task_size_TA)
        Flow_TA = (buy_obj + sell_obj) * 50
    ...
    def getActionMsgs(self, action: Dict, state: EnvState, params: EnvParams, agentType: str):
        '''
        ...
        if agentType == 'OOE':
            ...
            trader_ids=jnp.ones((self.n_actions_TA,),jnp.int32)*self.trader_unique_id_TA #This agent will always have the same (unique) trader ID (-9001)
            order_ids=jnp.ones((self.n_actions_TA,),jnp.int32)*(self.trader_unique_id_TA-state.customIDcounter)-jnp.arange(0,self.n_actions_TA) #Each message has a unique ID
            sides = jax.lax.cond(action[1] != 0,lambda _: -1 * jnp.ones((self.n_actions_TA,), jnp.int32),lambda _: jnp.ones((self.n_actions_TA), jnp.int32),operand=None)
            types=jnp.ones((self.n_actions_TA,),jnp.int32)
            times=jnp.resize(state.time+params.time_delay_obs_act,(self.n_actions_TA,2)) #time from last (data) message of prev. step + some delay
        ...
        # --------------- 02 info for deciding prices TAgent ---------------
        B = best_ask
        S = best_bid
        H = 0
        # --------------- 02 info for deciding prices TAgent ---------------
        ...
        def normal_order_logic_TA(state: EnvState, action: jnp.ndarray):
            quants = action.astype(jnp.int32) 
            prices = jnp.asarray((B, S, H), jnp.int32)
            return quants, prices
        ...
        if agentType == 'OOE':
            ...
            quants, prices = normal_order_logic_TA(state, action)
        # --------------- 03 Limit/Market Order (prices/qtys) ---------------
