...
class ExecutionEnv(BaseLOBEnv):
    ...
    def step_env(
        ...
        def getActionMsgsTA(action: Dict, state: EnvState, params: EnvParams, order_id=[1]) -> chex.Array:
            self_n_actions_TA = 1
        ...
        messageForMike = jax.vmap(getActionMsgsTA, in_axes=(0, None, None, 0))(actions_TA_, state, params, IDs)
        ...
        #jax.debug.print('Inventory List: {}', quant_inventory_arr)
        #jax.debug.print('PnL List TAgents: {}', PnL_arr)
        ...
        state = EnvState(asks,bids,trades,bestasks,bestbids,state.init_time,time,state.customIDcounter+self.n_actions,state.window_index,\
            ...
            state.max_steps_in_episode, RelPriceDelta,
            self.task_size_TA, quant_buy_arr, quant_sold_arr, quant_inventory_arr,
            mid_price,
